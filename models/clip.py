...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embed = self.resize_pos_embed(pos_embed, (H, W))  # NC(1+HW)
        x = x + pos_embed.to(x.dtype)  # NC(1+HW)
        x = x.permute(2, 0, 1)  # (1+HW)NC
...
class VisionTransformer(nn.Module):
    ...
    def resize_pos_embed(self, pos_embed, input_shpae):
        ...
        pos_embed_weight = torch.flatten(pos_embed_weight, 2).transpose(1, 2).squeeze(0) # [L, C]
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print("x:", x.shape, " PE:", self.positional_embedding.shape, " input shape:", self.input_resolution)
...
def build_model(state_dict: dict, txt_length: int, fus:bool=True):
    ...
    # logger.info("++++++ delete visual pretrained parameters +++++++")
    # for key in list(state_dict.keys()): # 删去视觉感知的权重
    ...
    # for name, param in model.named_parameters(): # 冻结text encoder
